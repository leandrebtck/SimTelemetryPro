...
Uses the three AC shared memory pages:
  Local\\acpmf_physics   - physics data (60 Hz)
  Local\\acpmf_graphics  - game/session data
  Local\\acpmf_static    - static car/track info
"""
...
# ---------------------------------------------------------------------------
# ctypes structures matching the official AC SDK
# ---------------------------------------------------------------------------
...
class ACReader(BaseReader):
    ...
    def __init__(self, poll_hz: int = 60):
        ...
        self._phys_buf   = SPageFilePhysics()
        self._gfx_buf    = SPageFileGraphics()
        self._static_buf = SPageFileStatic()
    ...
    def read_frame(self) -> Optional[TelemetryFrame]:
        ...
        # AC status: 0=AC_OFF, 1=AC_REPLAY, 2=AC_LIVE, 3=AC_PAUSE
        if g.status == 0:
            return None
        ...
        return TelemetryFrame(
            ...
            gear=p.gear - 1,  # AC: 0=R,1=N,2=1st → normalize to -1=R,0=N,1=1st
            rpm=float(p.rpms),
